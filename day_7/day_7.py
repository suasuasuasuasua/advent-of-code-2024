...
with open(input_file, "r") as file_in:
    equations = [
        [int(o.strip(":")) for o in row.split()] for row in file_in.readlines()
    ]


def test_operators(acc: int, cur: tuple[int, Callable]) -> int:
    operand, op = cur

    if op == operator.concat:
        return int(op(str(acc), str(operand)))
    else:
        return op(acc, operand)
...
for equation in equations:
    ...
    # for n operands, we can slot in n-1 operators - 2^n-1
    # cartesian product gives us all the relevant combos
    p1_combos = itertools.product(
        [operator.add, operator.mul], repeat=len(operands) - 1
    )
    p2_combos = itertools.product(
        [operator.add, operator.mul, operator.concat], repeat=len(operands) - 1
    )

    for combo in p1_combos:
        result = functools.reduce(test_operators, zip(rest, combo), first)

        # Count the combos that result in the target (don't double count!)
        if result == target:
            p1 += result
            break

    for combo in p2_combos:
        result = functools.reduce(test_operators, zip(rest, combo), first)
