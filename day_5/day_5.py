...
with open(input_file, "r") as file_in:
    for row in file_in:
        row = row.strip()
        # Ignore the separating newline
        if not row:
            continue

        m = order_pattern.match(row)

        # Find the ordering definitions
        if m:
            first, second = m.groups()
            orderings[first].add(second)
        # Otherwise, save the safety manuals
        else:
            pages = row.split(",")
            manuals.append(pages)


def is_manual_valid(manual: list[str]) -> bool:
    for i, page in enumerate(manual):
        ...
        # Make sure that all previous pages are correct
        for p in prev:
            # To do this, we can reverse check that the current page is not in
            # any of the other previous pages' follow sets
            if not orderings[p].intersection([page]):
                return False

        # Make sure all next pages are correct by checking the current page's
        # follow set
        if not orderings[page].intersection(next):
            return False
...
def fix_manual(manual: list[str]) -> list[str]:
    ...
    for i, page in enumerate(manual):
        ...
        # Get the current page's follow set count by computing an intersection
        prev_inter = orderings[page].intersection(prev)
        next_inter = orderings[page].intersection(next)
        count = len(prev_inter | next_inter)

        fixed_manual[page] = count

    # Order by the highest priority page
    fixed_manual = [
        page
...
for manual in manuals:
    if is_manual_valid(manual):
        middle_page = int(manual[len(manual) // 2])
        p1 += middle_page
    else:
        fixed_manual = fix_manual(manual)
        middle_page = int(fixed_manual[len(fixed_manual) // 2])
        p2 += middle_page
